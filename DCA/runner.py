...
# This function is originally obtained from DCA repo (https://github.com/theislab/dca/blob/master/DCA/__main__.py)

def parse_args():
    parser = argparse.ArgumentParser(description='Autoencoder', formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    parser.add_argument('-i', '--input', type=str, default='input_count.csv', help='Input is raw count data in TSV/CSV '
    ...
    parser.add_argument('-o', '--output', type=str, default='imputed_count.csv', help='Output file')
    ...
    parser.add_argument('--result', type=str, default='mean.tsv', help='The tsv file to be returned from DCA output dir '
                             '(all/dispersion.tsv/mean.tsv/mean_norm.tsv/mode.tsv/pi.tsv/reduced.tsv).')
...
result_file = os.path.join("./dca_output", args.result)

if not os.path.exists(input_file):
    raise Exception("File `%s` not exists. Did you mount `/data` volume into docker?" % input_file)


from DCA import train

delattr(args, 'output')
result_arg = args.result
delattr(args, 'result')
args.input = input_file
if result_arg == 'all':
    args.outputdir = '/data/dca_output'
else:
    args.outputdir = './dca_output'
...
if result_arg != 'all':
    ...
    print("result file saved to `%s`" % output_file)
